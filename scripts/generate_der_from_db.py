...
if len(columns) != 19 or len(foreign) != 2:
...
def box(table, x, y, width):
    ...
    d.rounded_rectangle((x+9, y+11, x+width+9, y+height+11), radius=18, fill="#DFE8EC")
    d.rounded_rectangle((x, y, x+width, y+height), radius=18, fill="white", outline=LINE, width=3)
    d.rounded_rectangle((x, y, x+width, y+76), radius=18, fill=NAVY if table != "movimentacoes" else TEAL)
    ...
    for i, (name, kind, key, nullable, is_fk) in enumerate(rows):
        ...
        tag = "PK" if key == "PRI" else "FK" if is_fk else "UQ" if key == "UNI" else ""
        if tag:
            color = TEAL if tag == "PK" else "#2B6796" if tag == "FK" else "#84569B"
            d.rounded_rectangle((x+25, top+12, x+73, top+42), radius=6, fill=color)
            d.text((x+32, top+16), tag, font=font_tag, fill="white")
...
d.rounded_rectangle((77, 930, 2123, 1050), radius=16, fill="#E7F3F1")
d.text((105, 950), "Leitura do modelo", font=font_heading, fill=NAVY)
d.text((105, 1002), "PK = chave primária    FK = chave estrangeira    UQ = valor único    ·    Cada movimentação exige 1 produto e 1 usuário.", font=font_sub, fill=TEXT)
