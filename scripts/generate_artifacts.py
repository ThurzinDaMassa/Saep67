...
def documentation():
    story = [P("Documentação do sistema", "TitleLocal"),
             ...
             P("Sistema web local para controlar ferramentas e equipamentos manuais pelo navegador, registrar entradas e saídas e sinalizar estoque abaixo do mínimo."),
    ...
    story += [PageBreak(), P("Modelo de dados", "HeadingLocal"),
              P("Usuários (1:N) movimentações (N:1) produtos. Cada movimentação pertence a um usuário e a um produto. O saldo atual e o mínimo ficam em produtos. As chaves estrangeiras estão em movimentacoes."),
              ...
              P("Login → tela inicial → cadastro de produtos ou gestão de estoque → registro de entrada/saída → atualização de saldo → alerta quando necessário → consulta de histórico → logout."),
              P("Arquitetura web", "HeadingLocal"),
              P("O servidor HTTP do JDK atende em http://127.0.0.1:8080. As páginas HTML/CSS apresentam login, painel, produtos, estoque e histórico. WebApp controla rotas e sessão; Store usa JDBC com consultas parametrizadas e transações. Formulários autenticados incluem token contra requisições forjadas."),
...
def tests_pdf():
    ...
    story += [P("Verificações automatizadas executadas", "HeadingLocal"),
              P("LogicTest: 14 verificações passaram em 24/09/2026. IntegrationTest: 19 verificações passaram no banco. UiSmokeTest: 12 verificações passaram no aplicativo desktop opcional. WebIntegrationTest: 32 verificações HTTP passaram com MariaDB, incluindo login, proteção de sessão, validações, CRUD, alerta, histórico e logout. Os testes removem apenas seus produtos temporários."),
              P("Próxima validação necessária", "HeadingLocal"),
              P("Conferir manualmente o diálogo do navegador que confirma a exclusão. Os demais fluxos foram exercitados via HTTP; login, painel, produtos, estoque e histórico foram também inspecionados visualmente em capturas do navegador.")]
...
def infrastructure():
    story = [P("Infraestrutura do projeto", "TitleLocal"),
             table(["Componente", "Especificação"], [
                 ...
                 ("Gerenciamento de build", "run.ps1 compila com javac e inicia o site; build.gradle declara o driver JDBC. Gradle 9.0.0 local falhou ao iniciar serviços nativos."),
             ], [4*cm, 13.1*cm]),
             ...
             P("Passaram 14 testes de lógica, 19 verificações de integração, 12 verificações Swing e 32 verificações HTTP do site. O diálogo de confirmação de exclusão foi conferido no código HTML e merece um clique manual antes da entrega.")]
